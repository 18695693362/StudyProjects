Shader "CookbookShader/lesson3/MySpecType" {
	Properties {
	...
	SubShader {
		...
		inline fixed4 LightingPhong(SurfaceOutput s,fixed3 lightDir,fixed3 viewDir,fixed atten)
		{
			...
			float3 reflectDir = normalize(2*s.Normal*diffuse_ - lightDir);

			float spec = pow(max(0,dot(reflectDir,viewDir)),_SpecPower);
			float3 specColor = _SpecColor*spec;

			fixed4 c;
			c.rgb = (s.Albedo * _LightColor0.rgb * diffuse_) + (_LightColor0.rgb * specColor);
			c.a = 1.0;
			return c;
		}
