...
只考虑环境光的光照模型。
\begin{equation}  
color = objColor * ambientColor;
\end{equation}  
** Phong
考虑环境光，漫反射光，镜面反射光的光照模型。其中镜面反射按照视角和反射光夹角来计算。
shininess 为反光度
#+BEGIN_LaTeX
diffuse = objNormal . lightDir;
specular = clamp(reflect(lightDir) * viewDir, 0, 1);
color = objColor * ( ambientColor + diffuse * lightColor ) + specular^shininess * lightColor;
#+END_LaTeX 
*** 缺点
1. 当放射光方向和视角夹角大于 90 度时，specular 被设置为 0，当反光度比较小时，镜面半径会比较大，而 specular 被设置为 0 导致，镜面光照突然就消失了。
2. 反射光方向计算复杂
  #+BEGIN_LaTeX
  refectLightDir = (lightDir . objNormal) * objNormal * 2 - lightDir;
  #+END_LaTeX 
** BlinnPhong
考虑环境光，漫反射光，镜面反射光的光照模型。反射光方向取视线和光照方向的中心线，镜面反射按照中心线和法线的夹角来计算。
\begin{equation}  
diffuse = objNormal . lightDir;
specular = normalize(lightDir + viewDir) . objNormal;
color = objColor * ( ambientColor + diffuse * lightColor ) + specular^shininess * lightColor;
\end{equation}
